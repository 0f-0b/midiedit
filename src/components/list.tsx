      ...
      {elements.map((element, index) => {
        ...
        return <li key={index} className={selected ? "list-element selected" : "list-element"} aria-selected={selected}>
          ...
          <div className="list-element-buttons edit-buttons">
            <button onClick={() => onAdd(index, selectedIndex + +(index <= selectedIndex))}>+</button>
            {(!notEmpty || count > 1) && <button onClick={() => onDelete(index, Math.max(0, selectedIndex - +(index <= selectedIndex)))}>-</button>}
          </div>
        </li>;
      })}
      <div className="list-element-buttons edit-buttons">
